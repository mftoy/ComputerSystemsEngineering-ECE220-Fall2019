...
/* INTRO PARAGRAPH
  ...
  I implemented following algorithms for the required functions.

  is_leaf_node: If a node has parent and no left or right child then return 1, 0 otherwise.

  is_internal_node: if a node has children, then it should be internal return1. 0 otherwise.

  is_in_subtree: if b is NULL, return zero because we reached the tree root. if a and b
                are equal to each other then b is rooted by a. otherwise call the same function
                recursively for b's parent.

  rotate: sway w and h using the basic cswapping algorithm.

  recut:  If current node is V change it to H, otherwise change it to V b/s it is H.

  swap_module: because both nodes are leaf node, just change the module pointer using swap
                algorithm. Their module value changed

  swap_topology: For two pointer we need to change 2 pointer: child and parent. Using if statemnt
                found out if they are left or right child and then change parents.

  get_expression: Nothing changed everything is in postfix_traversal.

  postfix_traversal: If a node is not leaf get the expression for left subtree and right subtree.
                      After that, get the expression for the root. Update expression struct then nth pointer.

  get_total_resource: if a node is leaf pointer get their resource value. Otherwise add left child
                      and right child's resource and return it.

  init_slicing_tree:  I implemented the algorithm provided. First of all, a new node created.
                      Then, if this is the last node make that node a leaf node, otherwise assign
                      a right child with function argument. left child calls recursively.
...
int is_leaf_node(node_t* ptr) {
  ...
  if((ptr->parent != NULL)&&(ptr->left == NULL)&&(ptr->right == NULL))//if a node has a parent node but no child node then leaf
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if((ptr->left != NULL)&&(ptr->right != NULL))//if a node has children then internal node
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  if(b == NULL)//if b is NULL node or b reached to tree root recursively
    return 0;//then b cannot be sub tree, return 0

  if(a == b)//if a and b equals to each other
    return 1;//then b is rooted by a

  return is_in_subtree(a, b->parent);//otherwise go to b's parents and check if a is a parent
}
...
void rotate(node_t* ptr) {
  // TODO:
  if(is_leaf_node(ptr)){//pointer should be a leaf node
    int temp = ptr->module->w;//get a temp to width
    ptr->module->w = ptr->module->h;//change width
    ptr->module->h = temp;//change height
  }
}
...
void recut(node_t* ptr) {
  ...
  if(ptr-> cutline == V)//if cutline is V
    ptr-> cutline = H;//chenge it to H
  else//other wise it is H
    ptr-> cutline = V;//chenge it to V
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  module_t * temp = a->module;//get a pointer to a's module
  a->module = b->module;//change a's module
  b->module = temp;//change b's module
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t* a_parent = a->parent;//get a pointer to a's parent
  node_t* b_parent = b->parent;//get a pointer to b's parent
  if(a == (a_parent->left)){//if a is a left child
    a_parent->left = b;//make a's left child b
  }
  else if(a == (a_parent->right)){//if a is a right child
    a_parent->right = b;//make a's right child b
  }

  if(b ==( b_parent->left)){//if b is b left child
    b_parent->left = a;//make b's left child a
  }
  else if(b == (b_parent->right)){//if b is b left child
    b_parent->right = a;//make b's left child a
  }

  a->parent = b_parent;//change the child parent pointer
  b->parent = a_parent;//change child's parent pointer
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  if(!is_leaf_node(ptr)){//if pointer is not a leaf node
    postfix_traversal(ptr->left, nth, expression);//get postfix expression of left subtree
    postfix_traversal(ptr->right, nth, expression);//get postfix expression of right subtree
  }

  int i;//declare an integer
  i = *nth;//get the value of argument
  expression[i].module = ptr-> module;//assign module value to expression struct
  expression[i].cutline = ptr-> cutline;//assign cutline value to expression struct
  *nth = i+1;//increment the pointer
}
...
{
  // TODO:
  int sum =0;//define a sum parameter zero
  if(is_leaf_node(ptr))//if a node is leaf
    return ptr->module->resource;//then it has resource, return that resource

  sum = sum + get_total_resource(ptr->left) + get_total_resource(ptr->right);//otherwise, go to childs and check if what resource they have
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
    ...
    node_t* ptr = (node_t *)malloc(sizeof(node_t));//get a new node pointer

    if(n == num_modules -1){//if n is the last node, it means it is a leaf
      ptr->right = NULL;//leaf node does not have any child
      ptr->left = NULL;//leaf node does not have any child
      ptr->parent=par;//parent is provided in the function
      ptr->cutline = UNDEFINED_CUTLINE;//undef cutline for leaf node
      ptr->module = (module_t *)malloc(sizeof(module_t));//get a module pointer for new node
      ptr->module->idx = n;//assign module value to new node;
      return ptr;//and return
    }

    ptr->cutline = V;//cut line vertical
    ptr->module = NULL;//because cut node module is null
    ptr->parent = par;//assign parent to new child

    //Right child
    node_t* rightChild = (node_t *)malloc(sizeof(node_t));//get a node pointer to right child
    ptr->right = rightChild;//it has to be a leaf node
    rightChild->cutline = UNDEFINED_CUTLINE;//leaf node = no cutline
    rightChild->parent = ptr;//new child's parent is newly defined parent
    rightChild->left = NULL;//leaf node = no child
    rightChild->right = NULL;//leaf node = no child
    rightChild->module = (module_t *)malloc(sizeof(module_t));//get a pointer to new module
    rightChild->module->idx = n;//assign module value

    //leftChild
    ptr->left = init_slicing_tree(ptr,n+1);//call recursively for left child
    return ptr;//return new node pointer
